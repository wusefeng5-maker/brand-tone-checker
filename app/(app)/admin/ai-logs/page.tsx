...
export default async function AdminAiLogsPage({
  ...
  return (
    <AdminPage
      description={t.admin.aiLogsDescription}
      labels={t.admin}
      title={t.admin.aiLogs}
    >
      <form className="mb-6 grid gap-3 rounded-3xl border border-zinc-100 bg-white p-5 shadow-sm shadow-zinc-200/70 md:grid-cols-4">
        ...
        <div className="flex items-end">
          <AdminFilterButton
            ...
            pendingLabel={t.common.filtering}
          />
        </div>
      </form>
    ...
    <>
      <div className="overflow-hidden rounded-3xl border border-zinc-100 bg-white shadow-sm shadow-zinc-200/70">
          ...
          <table className="w-full min-w-[900px] text-left text-sm">
            <thead className="border-b border-zinc-100 bg-zinc-50 text-xs font-semibold uppercase tracking-normal text-zinc-500">
              <tr>
                ...
                <th className="px-5 py-4">{labels.admin.provider}</th>
                <th className="px-5 py-4">{labels.admin.model}</th>
                <th className="px-5 py-4">{labels.admin.status}</th>
                <th className="px-5 py-4">{labels.admin.errorMessage}</th>
              ...
              {logs.map((log) => (
                <tr key={log.id}>
                  <td className="px-5 py-4 font-medium text-zinc-950">
                    {log.user.email ?? labels.common.noEmail}
                  </td>
        ...
        {logs.length === 0 ? (
          <div className="p-5">
            <EmptyState
              description={labels.admin.emptyLogsDescription}
              title={labels.admin.emptyLogs}
            />
          </div>
        ) : null}
      </div>
